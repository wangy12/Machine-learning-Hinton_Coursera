...
% forward
    inputs = data.inputs;% 256*1000
    w_input_hid = model.input_to_hid;%7*256
    w_hid_output = model.hid_to_class;%10*7
    z1 = w_input_hid * inputs;%7*1000
    a1 = logistic(z1); % 1./(1+log(-z1));%7*1000
    z2 = w_hid_output*a1;%10*1000
    ...
    log_class_prob = z2 - repmat(class_normalizer, [size(z2, 1), 1]); % log of probability of each class. size: <number of classes, i.e. 10> by <number of data cases>
    class_prob = exp(log_class_prob);
    
% % backward propagation
    
    ret.hid_to_class = (class_prob - data.targets) * a1'/length(inputs(1,:)) + model.hid_to_class * wd_coefficient;
    
    ret.input_to_hid = ( w_hid_output'*(class_prob - data.targets).*(a1.*(1 - a1)) ) * inputs'/length(inputs(1,:)) + model.input_to_hid * wd_coefficient;
    
    
%     Theta1_grad = zeros(size(model.input_to_hid));
%     Theta2_grad = zeros(size(model.hid_to_class));
    
%     m = length(data.inputs(1,:));
% for t = 1:m
%     a_1 = data.inputs(:,t);% 256*1
%     z_2 = model.input_to_hid * a_1; %7*1
%     a_2 = logistic(z_2);% 7*1
%     z_3 = model.hid_to_class * a_2;%10*1
%     
%     class_normalizer = log_sum_exp_over_rows(z_3); % log(sum(exp of class_input)) is what we subtract to get properly normalized log class probabilities. size: <1> by <number of data cases>
%     log_class_prob = z_3 - repmat(class_normalizer, [size(z_3, 1), 1]); % log of probability of each class. size: <number of classes, i.e. 10> by <number of data cases>
%     class_prob = exp(log_class_prob);
%     
% %     a_3 = class_prob(:,t);%10*1
%     delta_3 = (class_prob - data.targets(:,t));%10*1
%     delta_2 = model.hid_to_class'*delta_3.*(a_2.*(1 - a_2));
%     Theta2_grad = Theta2_grad + delta_3 * a_2';%10*7
%     Theta1_grad = Theta1_grad + delta_2 * a_1';
% end
% 
% ret.hid_to_class = Theta2_grad/m;
% ret.input_to_hid = Theta1_grad/m;
